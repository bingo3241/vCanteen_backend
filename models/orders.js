...
function getInProgressV2(customerId) {
    ...
    inproglist.forEach(order => {
        res.push({"orderId": order.orderId, "orderName": order.orderName, "orderNameExtra": order.orderNameExtra, "orderPrice": order.orderPrice, "restaurantName": order.restaurantName, "orderStatus": order.orderStatus, "createdAt" : order.createdAt, "orderEstimatedTime": order.order_prepare_duration + order.vendor_queuing_time})
    }) 
    return res             
}
